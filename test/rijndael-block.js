...
function runTest(size, blocks, mode, ivRequired = false, keySize = 256) {
  ...
  for (let i = 0; i < TEST_COUNT; i++) {
    ...
    const key = crypto.randomBytes(keySizeByte);
    const iv = crypto.randomBytes(sizeByte);
    const plaintext = crypto.randomBytes(sizeByte * blocks);

    // Test encryption
    {
      // Known implementation
      const known = new Mcrypt('rijndael-' + size, mode);
      known.open(key, iv);
      const expected = known.encrypt(plaintext).toString('hex');
